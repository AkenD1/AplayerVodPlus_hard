...
public class CircularSeekBar extends View {
	...
	private Integer[] volumeDegree = { 270, 294, 318, 342, 6, 30, 54, 78, 102,
			126, 150, 174, 198, 222, 246, 270 };
	...
	public CircularSeekBar(Context context, AttributeSet attrs) {
		...
		mCirclePaint.setStyle(Paint.Style.STROKE); // �������� STROKE���� FILL ʵ��
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		int tempInt = innerCircle + 1 + ringWidth / 2;
		RectF rect2 = new RectF(center - tempInt, center - tempInt, center + tempInt, center + tempInt);//������
		...
		if (0 != volumePos % volumeDegree.length) {
			...
			canvas.drawText(String.valueOf(volumePos), center, center + 15,
					mTextPaint);
		} else {
			Bitmap bitmap = drawableToBitmap(getResources().getDrawable(
					R.drawable.p_volume_slient));
			canvas.drawBitmap(bitmap, center - 20, center - 20, mPaintDot);
		}

		int dotXpos = getXpos(innerCircle - 13, center - 8,
				volumeDegree[volumePos]);
		int dotYpos = getYpos(innerCircle - 13, center - 8,
				volumeDegree[volumePos]);
		mPaintDot.setARGB(255, 212, 225, 233);
		Bitmap bitmap = drawableToBitmap(getResources().getDrawable(
				R.drawable.p_volume_dot));
