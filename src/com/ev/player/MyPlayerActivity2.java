...
public class MyPlayerActivity2 extends Activity implements OnKeyListener,OnTouchListener, OnGestureListener {
	...
	private boolean isAutoSwichPlay = true;
	private boolean isForceUrl = true;
...
//		mOSDControl = new OSDControlBar(MyPlayerActivity2.this, mContainer, aPlayerAndroid, videoPlayView, mtop_linearLayout, tvOrPad);
	...
	private void StartPlay() {
		vodBuffer.setVisibility(View.GONE);//隐藏缓冲buffer
		mBigPauseLayOut.setVisibility(View.GONE);//隐藏暂停按钮
		vodLoadAnimView.setVisibility(View.VISIBLE);// 开始加载动画
		if (now_video != null) {
			now_httpUrl = now_video.getUrl();
			if (now_httpUrl.indexOf("http") == 0) {
				isForceUrl = false;
				if(isAutoSwichPlay){
					isAutoSwichPlay = false;
					bt_player();
				}
				else{
	...
	private void ForceTvToHttp(String Url) {
		...
		sb.append("switch_chan").append("&id=").append(chnnelId).append("&server=").append(streamIp).append("&userid=")
				...
				.append(MD5Util.getStringMD5_32(MACUtils.getMac() + DeviceFun.GetCpuId(MyPlayerActivity2.this)));
...
//				int readPosition = aPlayerAndroid.getBufferProgress();
	...
	private Timer mTimerUpdateStateTimer = new Timer();
	private TimerTask mTimerUpdateStateTask = new TimerTask() {
		@Override
		public void run() {
...
//			};
//		}
//		if (mTimerUpdateStateTimer != null && mTimerUpdateStateTask != null){
...
//			if(v == videoPlayView && !isLoadingVod && !isShowVideoList)
//				return false;
			if(isLoadingVod || isShowVideoList)
				return false;
	...
	private void registerAplayerLister() {
		...
		aPlayerAndroid.setOnOpenSuccessListener(new APlayerAndroid.OnOpenSuccessListener() {
			...
			public void onOpenSuccess() {
				...
				videoPlayView.refreshDrawableState();
				
				vodLoadAnimView.setVisibility(View.GONE);// 关闭加载动画
				isLoadingVod = false;// 更新加载状态
				mIsVodPause = false;
				mOSDControl.setPlayIcon(true);//重置暂停播放按钮
				setTitle();// 设置标题
				int i = (int) aPlayerAndroid.getDuration();
				mOSDControl.setTotalTime(i);// 设置总时长

				aPlayerAndroid.play();// 开始播放
				aPlayerAndroid.setConfig(CONFIGID.SUBTITLE_FILE_NAME,
						"http://vidzi.tv/srt/01168/8ivual9kl5bd_English.srt");// 设置外挂字幕文件
		...
		aPlayerAndroid.setOnOpenCompleteListener(new APlayerAndroid.OnOpenCompleteListener() {
			...
			public void onOpenComplete(boolean isOpenSuccess) {
				...
				String openResultStr = isOpenSuccess ? "Success" : "Failed";
		...
		aPlayerAndroid.setOnPlayCompleteListener(new APlayerAndroid.OnPlayCompleteListener() {
			...
			public void onPlayComplete(String playRet) {
				...
				if (playRet.equals(APlayerAndroid.PlayCompleteRet.PLAYRE_RESULT_COMPLETE)){
					isAutoSwichPlay = true;
					play2WichVod(false);
				}
				//打开失败
				if (playRet.equals(APlayerAndroid.PlayCompleteRet.PLAYRE_RESULT_OPENRROR)){
					 Toast.makeText(MyPlayerActivity2.this, "PLAYRE_RESULT_OPENRROR",Toast.LENGTH_LONG).show();
				}
				if (playRet.equals(APlayerAndroid.PlayCompleteRet.PLAYRE_RESULT_SEEKERROR)){
					Toast.makeText(MyPlayerActivity2.this, "PLAYRE_RESULT_SEEKERROR",Toast.LENGTH_LONG).show();				
				}
				if (playRet.equals(APlayerAndroid.PlayCompleteRet.PLAYRE_RESULT_READEFRAMERROR)){
					Toast.makeText(MyPlayerActivity2.this, "PLAYRE_RESULT_READEFRAMERROR",Toast.LENGTH_LONG).show();
				}
				if (playRet.equals(APlayerAndroid.PlayCompleteRet.PLAYRE_RESULT_CREATEGRAPHERROR)){
					Toast.makeText(MyPlayerActivity2.this, "PLAYRE_RESULT_CREATEGRAPHERROR",Toast.LENGTH_LONG).show();
				}
				if (playRet.equals(APlayerAndroid.PlayCompleteRet.PLAYRE_RESULT_DECODEERROR)){
					Toast.makeText(MyPlayerActivity2.this, "PLAYRE_RESULT_DECODEERROR",Toast.LENGTH_LONG).show();
				}
				//硬件解码出错
				if (playRet.equals(APlayerAndroid.PlayCompleteRet.PLAYRE_RESULT_HARDDECODERROR)){
					Toast.makeText(MyPlayerActivity2.this, "PLAYRE_RESULT_HARDDECODERROR",Toast.LENGTH_LONG).show();
				}
			}
		});
		/*
...
//		Toast.makeText(MyPlayerActivity.this, this.getResources().getText(R.string.back_txt),Toast.LENGTH_LONG).show();
		Toast.makeText(MyPlayerActivity2.this, this.getResources().getString(R.string.back_txt1),Toast.LENGTH_LONG).show();
		setBackClock();
	}
